var request = require(__dirname + '/requestTest')
  ...
  describe('GET /countries/bra/players', function() {
    before(function() {
    ...
    it('respond with json', function(done) {
      ...
      request(app).get('/countries/bra/players', 200, expectedResponse, done);
    });
  });

  describe('GET /countries/ita/players', function() {
    before(function() {
    ...
    it('respond with json', function(done) {
      ...
      request(app).get('/countries/ita/players', 200, expectedResponse, done);
    });
  });

  describe('GET /countries/not_found/players', function() {
    before(function() {
      nock(url) .get('/XXXplayerbasicdata.xml').reply(404, '<html><h1>Not Found</h1><p>The resource could not be found.</p></html>');
    });
    it('respond with json', function(done) {
      request(app).get('/countries/xxx/players', 404, '404: Page not Found', done);
    });
  });
